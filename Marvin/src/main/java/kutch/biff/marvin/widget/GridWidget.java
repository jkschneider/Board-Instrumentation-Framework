...
public class GridWidget extends BaseWidget {
    ...
    public void CheckSizingBounds(int depth) {
        ...
        if (configD.getWidth() > 0) {
            if (configD.getWidth() < local.getWidth() && local.getWidth() - configD.getWidth() > depth * 2) // *2 is to
            // account
            // for
            // borders
            // in debug
            // mode
            {
                LOGGER.warning("Grid Widget[" + Integer.toString(getWidgetNumber()) + "] configured width is "
                        + Integer.toString((int) configD.getWidth()) + ", but real width is "
                        + Integer.toString((int) local.getWidth()));
            }
        }
        if (configD.getHeight() > 0) {
            if (configD.getHeight() < local.getHeight() && local.getHeight() - configD.getHeight() > depth * 2) {
                LOGGER.warning("Grid Widget[" + Integer.toString(getWidgetNumber()) + "]configured Height is "
                        + Integer.toString((int) configD.getHeight()) + ", but real height is "
                        + Integer.toString((int) local.getHeight()));
            }
        }
    ...
    public boolean handlePercentageDimentions() {
        if (gethGapPercentOfParentGrid() > 0) {
            ...
            if (parentWidth == 0) {
                LOGGER.severe("Widget [" + getName()
                        + "] hGap specified as percentage of parent grid - but parent grid width not specified.");
                return false;
            }
        ...
        if (getvGapPercentOfParentGrid() > 0) {
            ...
            if (parentHeight == 0) {
                LOGGER.severe("Widget [" + getName()
                        + "] vGap specified as percentage of parent grid - but parent grid width not specified.");
                return false;
            }
    ...
    public boolean HandleWidgetSpecificSettings(FrameworkNode widgetNode) {
        if ("PaddingOverride".equalsIgnoreCase(widgetNode.getNodeName())
            ...
            try {
                ...
                LOGGER.severe("Invalid PaddingOverride/Padding configuration: " + strTop + "," + strBottom + ","
                        + strLeft + "," + strRight);
                return false;
            }
        }
    ...
    /**
     * The Grids do alignment different than widgets (kewl eh?) So have an override
     * fn to deal with it.
     *
