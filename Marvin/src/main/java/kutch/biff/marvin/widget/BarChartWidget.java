...
public class BarChartWidget extends LineChartWidget {
    ...
    private void displayLabelForData(XYChart.Data<String, Number> data) {
        ...
        node.parentProperty().addListener((ObservableValue ov, Parent oldParent, Parent parent) -> {
            Group parentGroup = (Group) parent;
            parentGroup.getChildren().add(dataText);
        });

        node.boundsInParentProperty().addListener((ObservableValue ov, Bounds oldBounds, Bounds bounds) -> {
    ...
    private void setupListenersForSingleSource(DataManager dataMgr) {
        ...
        dataMgr.AddListener(getMinionID(), getNamespace(),
                new MarvinChangeListener(get__dataIndex(), get__dataIndexToken()) {
                    @Override
                    public void onChanged(String newVal) {
                        ...
                        if (strList.length != getxAxisMaxCount()) {
                            LOGGER.info("Received " + Integer.toString(strList.length)
                                    + " items for a Bar Chart, so changing chart.");
