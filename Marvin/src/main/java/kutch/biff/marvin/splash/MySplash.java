...
public class MySplash {
    ...
    public void init() {
        ...
        splashLayout.setStyle(
                "-fx-padding: 5; -fx-background-color: darkgray; -fx-border-width:5; -fx-border-color: darkslategray;");
    ...
    public void start(Stage parentStage) {
        ...
        _splashAnimationTimer = new AnimationTimer() // can't update the Widgets outside of GUI thread, so this is a
                // little worker to do so
        {
            @Override
            public void handle(long now) {
                if (null == AppVisualBounds && null != Configuration.getConfig()) {
                    if (Configuration.getConfig().isPrimaryScreenDetermined()) {
                        ...
                        /*
                         * double aboutBoxW = _Stage.getWidth()/2; double configW
                         * =Configuration.getConfig().getWidth()/2; double visM =
                         * AppVisualBounds.getMinX(); double visWid = AppVisualBounds.getMaxX();
                         */
                        ...
                        // Configuration.getConfig().getHeight()/2- _Stage.getHeight()/2);
                    }
                    return;
                }
